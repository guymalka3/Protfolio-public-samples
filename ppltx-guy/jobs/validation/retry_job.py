...
"""
---------PATH---------
C:/workspace/ppltx-guy/jobs/validation/retry_job.py
...
python3 ./jobs/validation/retry_job.py guy-ppltx
...
step_id = 0
env_type = 'daily'
log_table = f"{project_id}.g_logs.logs_daily"
...
def find_job_path(file_name: str) -> Path:
    """
    Find the full repo-relative path to a job script by filename.
    Searches all job directories except 'validation'.
    """
    JOBS_DIR = REPO_ROOT / "jobs"

    # Search all job subdirectories (appsflyer, gaming, etc.)
    for job_dir in JOBS_DIR.iterdir():
        if job_dir.is_dir() and job_dir.name != "validation":
            # Recursive search for matching filename
            for py_file in job_dir.rglob(file_name):
                if py_file.is_file():
                    return py_file.resolve()  # Full absolute path

    # Fallback: try in current script dir
    candidate = SCRIPT_DIR / file_name
    if candidate.is_file():
        return candidate.resolve()

    raise FileNotFoundError(f"Job script '{file_name}' not found in jobs/ directories")
    ...
    if not flags.dry_run:
        ...
        for i in range(days_from_last_run, 0, -1):
            try:
                ...
                python_cmd = f"python3 {message} --days-back {i}"
                subprocess.run(python_cmd,check=True, shell=True, text=True, timeout=3600)
                msg = f"Job Re-run completed\n*****{y_m_d}*****\n"
                print(msg)
                send_slack_alert(f":bell:Retry Job has ran\n{python_cmd}\n{msg}")
            except subprocess.CalledProcessError as e:
                err_msg = f"\nJob Re-run failed with return code: {e.returncode},\n {e.stderr}\n{e.stdout}"
                print(err_msg)
