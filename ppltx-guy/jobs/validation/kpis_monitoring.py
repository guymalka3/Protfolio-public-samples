...
"""
---------PATH---------
C:/workspace/ppltx-guy/jobs/gaming/kpis_monitoring.py
...
# ----- Get KPIs Job Config -----
kpi_config_path = SCRIPT_DIR / f"config/validation_config.json"
kpis_configuration = readJsonFile(kpi_config_path)
...
for kpis_group_name, group_conf in kpis_configuration.items():
    ...
    for kpis_name, kpis_conf in group_conf[f"{etl_name}"].items():
        if not kpis_conf["isEnable"]:
            continue
        print(kpis_name)
        # query_sql = readFile(SCRIPT_DIR / f"queries/{etl_name}_{kpis_name}_alert.sql")
        sql_name = kpis_conf.get("sql")
        query_sql = readFile(SCRIPT_DIR / f"queries/{sql_name}.sql")
        ...
        #write query to log
        writeFile(LOGS_PATH / f"{kpis_name}_monitoring.sql", query)
        ...
        if not flags.dry_run:
            try:
                ...
                header (f"""Hi BI Developer we have a problem in {kpis_name}
                \nOpen file {str(LOGS_PATH)}/{kpis_name}_monitoring_error.md""")
                print(msg_error)
                send_slack_alert(msg_error)
                writeFile (LOGS_PATH / f"{kpis_name}_monitoring_error.md", msg_error)

# if the df has values send a message
if (df_all[alert_columns]).any():
    # extract only the rows with raising_flag = True
    error_msg = f"[{etl_name} Alert]"
