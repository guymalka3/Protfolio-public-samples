...
--creates inc table of the last 2 days
CREATE OR REPLACE TABLE
  `{project_id}.{dataset_dst}.{table_dst}_inc` --  `subpltx-dev.g_appsflyer.daily_campaign_panel_inc`
(
...
--deletes from overlapping days in daily panel
DELETE FROM `{project_id}.{dataset_dst}.{table_dst}` --`subpltx-dev.g_appsflyer.daily_campaign_panel`
WHERE
  {partition_att} >= "{date}";

-- Insert new data into table from inc table
INSERT INTO `{project_id}.{dataset_dst}.{table_dst}` --`subpltx-dev.g_appsflyer.daily_campaign_panel`
SELECT
  *
FROM
  `{project_id}.{dataset_dst}.{table_dst}_inc`; --`subpltx-dev.g_appsflyer.daily_campaign_panel_inc`
